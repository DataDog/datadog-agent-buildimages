...
def _get_pipeline_status(ctx: Context, pipeline_id, job_token):
    get_status_cmd = f"curl --header \"JOB-TOKEN: {job_token}\" https://gitlab.ddbuild.io/api/v4/projects/1856/pipelines/{pipeline_id}"
...
def trigger_child_pipeline(ctx: Context, variable):
    ...
    pipeline_status = _get_pipeline_status(ctx, metadata["id"], CI_JOB_TOKEN)
