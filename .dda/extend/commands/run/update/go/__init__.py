...
def _get_msgo_sha256(version, msgo_patch) -> OrderedDict[Platform, str]:
    return _get_expected_sha256(f"{version}-{msgo_patch}", f"https://github.com/DataDog/msgo/releases/download/v{version}-{msgo_patch}")
...
def cmd(app: Application, *, version: str, msgo_patch: str, check_archive: bool) -> None:
    ...
    if check_archive:
        try:
            ...
            _check_archive(app, version, msgo_shas, f"https://github.com/DataDog/msgo/releases/download/v{version}-{msgo_patch}")
